pub mod ai_new {
    ...
    pub async fn secondary_ada_builder() -> Result<(), Box<dyn Error>> {
        ...
        let mut lock: std::io::StdoutLock<'static> = stdout().lock();
    ...
    //     let location: &str = function_args["location"].as_str().unwrap();
    //     let unit: &str = function_args["unit"].as_str().unwrap_or("fahrenheit");
    //     let function = available_functions.get(name).unwrap();
    //     let function_response: Value = function(location, unit); // call the function
